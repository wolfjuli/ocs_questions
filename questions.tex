...
The conditional gradient method generates the point \(\kth{\bar{x}}\) by finding the feasible point
furthest away from \(\kth{x}\) along the negative gradient direction:
\begin{align*}
...
\(h(x^* + d) = 0\) (i.e., the constraint is still fulfilled).
...
\begin{align*}
  ...
  A x^* &= 0.
\end{align*}
Inserting the second into the first gives us
\begin{align*}
  A(y - \T{A} \lambda) &= 0 \\
  Ay &= A \T{A} \lambda \\
  \lambda = (A \T{A})^{-1} A y,
