...
\summary{Strong Convexity}\label{s:strong_convexity}
...
\optionalsummary{General Lower Bounds for Convex Functions}
...
\item Smooth convex functions: for any \(k\) with \(1 \leq k \leq \frac{1}{2}(n - 1)\), \(L > 0\),
  ...
  \item \(\displaystyle f(\kth{x}) - f(x^*) \geq \frac{3L \enVert[1]{\kth[0]{x} - x^*}}{32 (n+1)^2}\),
  \end{enumerate}
\item Strongly convex functions: for any \(\kth[0]{x} \in \RR^n\), \(\mu > 0\), and \(L > \mu\),
  ...
  have
  \begin{enumerate}
  \item \(\displaystyle \enVert[1]{\kth{x} - x^*} \geq
    \left( \frac{\sqrt{L/\mu} - 1}{\sqrt{L/\mu} + 1} \right)^k \enVert[1]{\kth[0]{x} - x^*}\)
...
% \begin{equation*}
%   f(\kth{x}) - f(x^*) \geq \frac{3\lambda_{\text{max}} \enVert[1]{\kth[0]{x} - x^*}}{32 (n+1)^2}.
% \end{equation*}
...
the optimal first order method for quadratic problems: if \(0 \preceq \mu \preceq Q \preceq L\) is
positive definite, then we have a strongly convex problem and
\begin{equation*}
  \enVert[1]{\kth{x} - x^*} \leq 2\sqrt{L/\mu} \left( \frac{\sqrt{L/\mu}-1}{\sqrt{L/\mu}+1} \right)
  \enVert[1]{\kth[0]{x} - x^*},
\end{equation*}
and if \(0 \preceq Q \preceq L\) is positive semidefinite, we have a smooth problem with
\begin{equation*}
  f(\kth{x}) - f(x^*) \leq \frac{L \enVert[1]{\kth[0]{x} - x^*}^2}{2 (2k + 1)^2}.
\end{equation*}
Furthermore, in this case, we have the easy characterization \(L = \lambda_{\text{max}}(Q)\) and
\(\mu = \lambda_{\text{min}}(Q)\).
...
The following statement can be given globally for genereal functions: if
\(f \in \Strong{\mu}{L}{2}{1}\), and \(\alpha\), \(\beta\) are chosen as
\begin{align*}
...
\begin{equation*}
  ...
  \enVert{\begin{pmatrix}
    ...
    \end{pmatrix}}.
\end{equation*}
By this, the heavy ball method is optimal for strongly convex functions (\(\mu > 0\)), but does
not work well if \(\mu = 0\) (then, we can't choose the parameters in the shown way).
...
If we know the problem to be strongly convex, ie., \(f \in \Strong{\mu}{L}{1}{1}\), we can use a
constant value of
\begin{equation*}
