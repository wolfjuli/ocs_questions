...
of second-order methods, like Newton's.  They work for \emph{quadratic problems} of the form
\begin{equation*}
  \min_{x \in \RR^n} f(x) = \frac{1}{2} \T{x}Qx - \T{b}x.
\end{equation*}
Originally, they were developed for symmetric and positive definite \(Q\), but they also work for
solving linear systems of the form \(Qx = b\), where \(Q\) is not neccessarily symmetric, but just
invertible.
...
The conjugate gradient method is the most important conjugate direction method.  It is obtained by
...
\begin{equation*}
  \kth{\beta} = \frac{\T{(\kth{g})}\kth{g}}{\T{(\kth[k-]{g})}\kth[k-1]{g}}
\end{equation*}
