...
\summary{Calculating Eigenvalues by Hand}
...
\summary{Convex Functions}\label{s:convex-functions}
...
\end{equation*}
This means that the graph always lies below a secant.  Equivalent to this, for sufficiently
differentiable functions, are the first-order condition
\begin{equation*}
  f(x) \geq f(y) + \T{\nabla f(y)}(x - y), \quad \forall x, y \in X
\end{equation*}
(the graph lies above any tangent) and the second-order condition
\begin{equation*}
  \nabla^2 f(x) \succeq 0 \quad \forall x \in X
\end{equation*}
(curvature is everywhere nonnegative).  We speak about \emph{strict convexity} when the inequalities
...
becomes a sufficient condition: assume \(\nabla f(x^*) = 0\), but \(x^*\) were not optimal, so there is a
  ...
  graph lies above any tangent; see Summary~\ref{s:convex-functions}),
\begin{align*}
...
\item Smooth convex functions: for any \(k\) with \(1 \leq k \leq \frac{1}{2}(n - 1)\), \(L > 0\),
  ...
  \begin{enumerate}
  \item \(\displaystyle \enVert[1]{\kth{x} - x^*}^2 \geq \frac{1}{8} \enVert[1]{\kth[0]{x} - x^*}^2\)
  \item \(\displaystyle f(\kth{x}) - f(x^*) \geq \frac{3L \enVert[1]{\kth[0]{x} - x^*}^2}{32 (k+1)^2}\),
  \end{enumerate}
...
\begin{equation*}
  \enVert[1]{\kth{x} - x^*}^2 \leq 4(L/\mu) \left( \frac{\sqrt{L/\mu}-1}{\sqrt{L/\mu}+1} \right)^{2k}
