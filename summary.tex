...
the \emph{Gram-Schmitd procedure}.  Therefore, we first define the projection operator (in the
\(Q\)-metric):
\begin{equation*}
...
\begin{align*}
  \kth[0]{d} &= \kth[0]{\xi} \\
  \kth[i+1]{d} &= \kth[i+1]{\xi} - \sum_{m=0}^i \proj_{\kth[i]{d}}(\kth[i]{\xi}) \\
             &= \kth[i+1]{\xi} - \sum_{m=0}^i
               \frac{\T{(\kth[i]{d})} Q \kth[i]{\xi}}{\T{(\kth[i]{d})} Q \kth[i]{d}} \kth[i]{d}.
\end{align*}
