...
\begin{align*}
  ...
  \nabla f(x(t)) &\approx \nabla f(\kth{x}).
\end{align*}
Rearranging these terms results in exactly this form, where \(\alpha\) and \(\beta\) are to be
chosen. If \(f\) is a quadratic problem, we can solve for them analytically:
\begin{equation*}
...
By this, the heavy ball method is optimal for smooth and strictly convex functions (\(\mu > 0\)),
