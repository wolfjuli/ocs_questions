...
Special cases of this are the \emph{Euclidean norm} (\(p = 2\)) and the \emph{Manhatton norm} (\(p = 1\)):
\begin{equation*}
...
\label{s:mu-strong_convexity} A function is \emph{\(\mu\)-strongly
  convex}\footnote{\url{https://en.wikipedia.org/wiki/Convex_function\#Strongly_convex_functions}}
if for all \(x, y \in X\) and \(\alpha \in [0,1]\):
\begin{equation*}
  f(\alpha x + (1 - \alpha) y) \leq \alpha f(x) + (1 - \alpha) f(y)
...
(\ref{s:mu-strong_convexity}) in the differential formulation, which provides the analog lower
bound.
