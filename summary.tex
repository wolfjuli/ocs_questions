...
\end{equation*}
Since we can also choose \(\Delta x\) to have the opposite sign, it holds that
\begin{equation*}
...
Newton's method has the property that it is invariant under affine cooridinate changes; ie., if we
exchange \(x = Sy\) for an invertible \(S\), the generated steps will remain the same.  First,
observe that
\begin{align*}
  \nabla_y f(Sy) &= \T{S} \nabla f(Sy), \\
  \nabla_y^2 f(Sy) &= \T{S} \nabla^2 f(Sy) S.
\end{align*}
Now, the local Taylor expansion becomes
\begin{align*}
  \kth{f}(Sy) &= f(S\kth{y}) + \T{(\T{S} \nabla f(Sy))}(y - \kth{y}) \\
  &\qquad + \frac{1}{2} \T{(y - \kth{y})} \nabla^2 f(S\kth{y})(y - \kth{y}),
\end{align*}
which we can minimize at \(\kth[k+1]{y}\) to get the update rule:
\begin{align*}
  \nabla \kth{f}(\kth[k+1]{y}) &= \T{S} \nabla f(S\kth{y}) + \T{S} \nabla^2
                                 f(S\kth{y})S(\kth[k+1]{y}
                                 - \kth{y}) \overset{!}{=} 0 \\
  \Rightarrow\quad \kth[k+1]{y} &= \kth{y} - \left( \T{S}\nabla f(S\kth{y})S \right)^{-1} \T{S}
                                  \nabla f(S\kth{y}) \\
                               &= \kth{y} - S^{-1} \nabla^2 f(S\kth{y})^{-1} (\T{S})^{-1}
                                 \T{S} \nabla f(S\kth{y}) \\
  \Rightarrow\quad S\kth[k+1]{y} &= S\kth{y} - \nabla^2 f(S\kth{y})^{-1} \nabla f(S\kth{y})
\end{align*}
\todo{this seems weird...}
...
a model function \(h\) with parameters \(x\), samples \(y_i\), and targets and \(z_i\).
...
\end{align*}
\begin{align*}
  \Rightarrow\hspace{1.125cm} 0 &\overset{!}{=} \frac{1}{2} \Big(\enVert[1]{g(\kth{x})}^2
