...
For matrices, we can consider so-called \emph{operator norms}.  An operator \(A: X \to Y\) between
normed spaces is called bounded by a number \(C\) if for all \(x \in X\)
\begin{equation*}
...
if there is value \(L \geq 0\), called \emph{Lipschitz constant}, such that for all
\(x, y \in \RR^n\)
\begin{equation*}
  \enVert{f(x) - f(y)} \leq L \enVert[1]{x - y}.
\end{equation*}
for some norm.  If \(f\) is differentiable, this is equivalent to
\begin{equation*}
...
A differnentiable function \(\RR \to \RR\) is Lipschitz continuous with \(L = \sup_x |g'(x)|\) if
...
(Summary~\ref{s:mu-strong_convexity}) in the differential formulation, which provides the analog
lower bound.
