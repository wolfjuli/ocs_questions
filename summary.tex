...
\begin{equation*}
  ...
  + o(\,\enVert{x - x_0}^2).
\end{equation*}
% calclulating eigenvalues by hand: http://math.harvard.edu/archive/21b_fall_04/exhibits/2dmatrices/index.html
...
Special cases of this are the \(Euclidean norm\) (\(p = 2\)) and the \(Manhatton norm\) (\(p = 1\)):
\begin{equation*}
...
there are the \(\ell_\infty\)- or \(maximum-norm\) and the \(\ell_0\)-quasinorm:
\begin{equation*}
  \lVert x \lVert_\infty = \max_i \{|x_i|\} \quad
\text{and} \quad \lVert x \lVert_\infty = \card \{i \mid x_i \neq 0\}.
\end{equation*}
...
For matrices, we can consider so-called \(operator norms\).  An operator \(A: X \to Y\) is called
bounded by a number \(C\) if for all \(x \in X\)
\begin{equation*}
...
Based on the operator norm, we can form the \(dual norm\) of a norm \(\lVert \cdot \rVert\):
\begin{equation*}
...
\begin{equation*}
  A = U \Sigma \T{V},
\end{equation*}
where \(U, V\) are orthogonal and \(\Sigma = \diag(\sigma_1, \ldots, \sigma_R)\).  It holds that
\(\sigma_i(A) = \sqrt{\lambda_i(A \T{A})}\).  Then
\begin{equation*}
  \lVert A \rVert_{S_p} = \left( \sum_{1 \leq i \leq N} \sigma_i(A)^p \right).
\end{equation*}
...
\begin{equation*}
  \lVert A \rVert_{F} = \sqrt{\sum_{1 \leq i \leq R} \sigma_i(A)} = \sqrt{\tr(A\T{A})} = \sqrt{\sum_{ij} a_{ij}^2}
\end{equation*}
...
If \(X\) is a convex set, then \(f: X \to \RR\) is called \emph{convex} if for all \(x, y \in X\) and
\(\alpha \in [0,1]\):
\begin{equation*}
...
A function is \emph{\(\mu\)-strongly
  convex}\footnote{\url{https://en.wikipedia.org/wiki/Convex_function\#Strongly_convex_functions}} if
...
\begin{equation*}
  f(x) \geq f(y) + \T{\nabla f(y)}(x - y) + \frac{\mu}{2}\enVert[1]{x - y}^2
...
\(m \to 0\), and is identical to the definition of a convex function when \(m = 0\).  On
\(X = \RR\), \(f\) is \(\mu\)-strongly convex if \(f''(x) \geq \mu > 0\) for all \(x\).


\summary{Lipschitz continuity}\label{s:lipschitz_continuity}
...
\summary{General Form of Optimization Problems}
...
this is that a the function must be convex in some neighbourhood for for a point to be ``at least as
...
\optionalsummary{Existence of Minimizers}

A crucial condition for the general existence of minimizers is the following: a function
\(f: X \subset \RR^N \to \RR \cup \{\infty\}\) is \emph{lower semicontinuous}, if for all sequences
\({x_k} \in X\) converging to \(x \in X\) we have that
\begin{equation*}
  f(x) \leq \liminf_{k \to \infty} f(x_k).
\end{equation*}
If \(f\) is such, we can prove by the Heine-Borel theorem that if \(X\) is closed and bounded, f has a global
minimizer.
...
\summary{Gradient Related Condition}\label{s:gradient_related_condition}
...
continuity (Summary~\ref{s:lipschitz_continuity}).
...
\summary{Derivation of Newton's Method from Taylor Expansion}\label{s:newton_method_derivation}
...
\summary{Gauss-Newton Method}\label{s:gauss_newton_method}
...
\summary{Levenberg-Marquardt Method}\label{s:levenberg_method}
...
\summary{Conjugate Gradient Method}
...
\summary{Application of CG to Nonlinear Problems}
...
\summary{Conditions of Optimality}
...
\summary{Projection on a Convex Set}
...
\summary{Equality Constrained Quadratic Problems}


\summary{Feasible Direction Methods}

Definition, alternative definition, step sizes, convergence


\summary{Conditional Gradient Method}


\summary{Gradient Projection Method}

+ scaled variants


\summary{Linear Programming with Equality Constraints}

Intro, as a sequence of quadratic problems


\summary{Affine Scaling Method}


\summary{Linear Programming with Inequality Constraints}
